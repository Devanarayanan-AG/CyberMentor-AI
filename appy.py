...
with gr.Blocks(title="CyberMentor AI Platform") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("🤖 AI Cybersecurity Tutor"):
            ...
            # UPGRADED: Added full layout file component accepting both PDFs and text files
            with gr.Row():
                upload_btn = gr.File(label="Upload Custom Syllabus/Textbook (.txt or .pdf)", file_types=[".txt", ".pdf"])
                upload_status = gr.Textbox(label="System Status", value="Awaiting documentation upload...", interactive=False)
            ...
            # Trigger backend data parsing when a new file hits the application frame
            upload_btn.change(
                fn=engine.process_user_upload,
                inputs=[upload_btn],
